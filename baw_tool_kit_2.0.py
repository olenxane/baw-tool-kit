...
import threading
import json
import bili_subscribe_down_mv4 as bsd
...
#配置变量
try:
    with open("baw_tool_kit_config.json", "r") as f:
        config = json.load(f)
except FileNotFoundError:
    print("配置文件不存在，使用默认值")
    config = {}
except json.JSONDecodeError:
    print("配置文件格式错误，使用默认值")
    config = {}
base_url = config.get("base_url", "https://api.deepseek.com/v1")
api_key = config.get("api_key", "sk-0414ce3d01f84549864818e1653b0a21")
model = config.get("model", "deepseek-v4-flash")
config_width = config.get("width", 320)
config_height = config.get("height", 240)
config_video_bitrate = config.get("video_bitrate", 700)
config_audio_bitrate = config.get("audio_bitrate", 192)
config_framerate = config.get("framerate", 21)
config_split_video_time = config.get("split_video_time", 300)
...
def extract_metadata(input_file):
    ...
    try:
        ...
        result = subprocess.run(ffprobe_cmd, shell=True, capture_output=True, text=True, encoding='utf-8', errors='replace')
...
def do_conversion(file_type):
    ...
    output_resolution = (int(width_entry.get() or config_width), int(height_entry.get() or config_height))
    video_bitrate = int(bitrate_entry.get() or config_video_bitrate)
    audio_bitrate = int(audio_bitrate_entry.get() or config_audio_bitrate)
    framerate = int(framerate_entry.get() or config_framerate)
    try:
        audio_block_size = int(22050 / framerate)
    except ZeroDivisionError:
        audio_block_size = 1050
    audio_quality = 0
    split_video_time = int(spilit_video_time_entry.get() or config_split_video_time)
    ...
    config.update({
        ...
        "framerate": framerate,
        "split_video_time": split_video_time
    })
    with open("baw_tool_kit_config.json", "w") as f:
...
def rewrite_aac_single(file):
    ...
    try:
        ...
        print(f"AAC/M4A转换完成）: {output_filename}")
    except subprocess.CalledProcessError as e:
        print(f"转换错误: {e}")
...
# llm设置面板
def llm_setting_panel():
    panel=Tk()
    panel.title("llm设置面板")
    panel.geometry("360x420")
    api_key_label=Label(panel,text="请输入api_key:")
    api_key_label.pack(pady=10)
    api_key_entry=Entry(panel,width=50)
    api_key_entry.insert(0, api_key)
    api_key_entry.pack(pady=10)
    base_url_label=Label(panel,text="请输入的base_url:")
    base_url_label.pack(pady=10)
    base_url_entry=Entry(panel,width=50)
    base_url_entry.insert(0, base_url)
    base_url_entry.pack(pady=10)
    model_label=Label(panel,text="请输入模型名称:")
    model_label.pack(pady=10)
    model_entry=Entry(panel,width=50)
    ...
    def save_llm_settings():
        ...
        with open("baw_tool_kit_config.json", "w") as f:
            json.dump(config, f, indent=4, ensure_ascii=False)
        print("llm设置已保存")
        messagebox.showinfo("提示", "LLM设置已保存")

    save_button = Button(panel, text="保存设置", command=save_llm_settings)
...
bitrate_entry.insert(0, str(config_video_bitrate))
...
audio_bitrate_entry.insert(0, str(config_audio_bitrate))
...
framerate_entry.insert(0, str(config_framerate))
...
spilit_video_time_entry.insert(0, str(config_split_video_time))
...
rotate_video = Checkbutton(root, text="是否旋转视频(如果要在横屏设备上放竖屏视频而且你不知道改成什么分辨率建议勾选)", variable=if_rotate)
rotate_video.pack()
...
open_bsd = Button(root, text="打开b站订阅下载工具", command=bsd.main)
open_bsd.pack()

llm_setting = Button(root, text="llm设置", command=llm_setting_panel)
llm_setting.pack()
